<script lang="ts">
  ...
  interface Props { liveState?: LiveState | null; initialQuery?: string }
  let { liveState = null, initialQuery = '' }: Props = $props();
  let query = $state('');
  $effect(() => { query = initialQuery; }); let tab = $state('all' as 'all' | 'confirmed' | 'needs-confirm');
