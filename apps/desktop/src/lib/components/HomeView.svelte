<script lang="ts">
  ...
  onMount(() => {
    ...
    // Same pairing source as App's camera client: the stored token from the
    // launcher fragment ('' in dev, where the Vite proxy owns the credential).
    homeApi = withEmptyPlanOn404(api ?? createHomeApi({ baseUrl: window.location.origin, serviceToken: bootstrapServiceToken(window) }));
    void loadSnapshot();
  });
</script>
...
<style>
  ...
  .home-tabs button[aria-selected='true'] { border-color: var(--ember); color: var(--ember); background: var(--panel); }
  .home-selection { margin: 0; color: var(--ink-mute); font: 11px var(--font-mono); }
  .home-selection strong { color: var(--ink); font-weight: 600; }
  .home-banner { display: flex; flex-wrap: wrap; gap: 10px; align-items: center; padding: 12px 14px; border: 1px solid var(--ember); border-radius: var(--radius-card); background: var(--panel); color: var(--ink); font-size: 12.5px; }
