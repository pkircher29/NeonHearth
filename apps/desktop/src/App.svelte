<script lang="ts">
  ...
  async function refreshAutomation() {
    if (automationApi) automationSnapshot = await automationApi.snapshot();
  }

  type Destination = 'pulse' | 'devices' | 'guard' | 'cameras' | 'home' | 'doctor' | 'automation';
  const nav: Array<[Destination, string, string]> = [['pulse', 'Pulse', '◒'], ['devices', 'Devices', '◌'], ['home', 'Home', '⌂'], ['automation', 'Automation', '⌘'], ['guard', 'Guard', '⌁'], ['cameras', 'Cameras', '□'], ['doctor', 'Doctor', '✚']];
  ...
  onMount(() => {
    ...
    automationApi = createAutomationApi(bootstrapServiceToken(window));
    let automationTimer: ReturnType<typeof setTimeout>;
    async function pollAutomation() {
      if (!active) return;
      if (view === 'automation' || view === 'home' || view === 'devices') {
        const results = await Promise.allSettled([refreshAutomation(), automationApi?.network()]);
        if (active) {
          if (results[0].status === 'rejected') automationSnapshot = null;
          networkDetails = results[1].status === 'fulfilled' ? results[1].value ?? null : null;
        }
      }
    ...
    return () => { active = false; clearTimeout(automationTimer); connection.stop(); };
  });
</script>
<svelte:head><title>NeonHearth — {view}</title></svelte:head>
<div class="app-shell"><header class="topbar"><div class="brand"><span class="brand-mark">✦</span><span>NeonHearth</span></div><nav aria-label="Primary">{#each nav as item}<button class:active={view === item[0]} type="button" onclick={() => { view = item[0]; moreOpen = false; }}><span class="nav-icon" aria-hidden="true">{item[2]}</span><span>{item[1]}</span></button>{/each}</nav><span class="topbar-note"><span class="secure-icon">⌾</span><span>Local only</span></span></header><header class="mobile-head"><div class="brand"><span class="brand-mark">✦</span><span>NeonHearth</span></div><span class="kicker">{view.toUpperCase()}</span></header><div class="app-body"><main>{#if pairingExpired}<section class="not-ready pairing-expired" role="alert"><span class="not-ready-mark" aria-hidden="true">⌁</span><p class="kicker">PAIRING / EXPIRED</p><h1>This window's pairing expired.</h1><p class="muted">Open NeonHearth from the Start menu to re-pair. Your network data never left this device — a fresh pairing simply issues this window a new key.</p></section>{:else if view === 'pulse'}<PulseView state={liveState}/>{:else if view === 'devices'}<DevicesView liveState={liveState} network={networkDetails}/>{:else if view === 'guard'}<GuardView state={liveState} client={cameraClient} />{:else if view === 'cameras'}{#if cameraClient}<CamerasView client={cameraClient} />{:else}<section class="not-ready"><p class="muted">Connecting to the local collector…</p></section>{/if}{:else if view === 'home'}<HomeView liveState={liveState} automation={automationSnapshot} network={networkDetails}/>{:else if view === 'automation'}{#if automationApi}<AutomationView api={automationApi} snapshot={automationSnapshot} refresh={refreshAutomation} openHome={() => view = 'home'}/>{/if}{:else if view === 'doctor'}{#if cameraClient}<DoctorView client={cameraClient} />{:else}<section class="not-ready"><p class="muted">Connecting to the local collector…</p></section>{/if}{/if}</main>{#if !pairingExpired}<aside class="events-rail"><div class="section-title"><h2>Live thread</h2><span class="live-pill"><i class="presence-dot {liveState.timeline.length ? 'online' : 'quiet'}" aria-hidden="true"></i> {liveState.timeline.length ? 'ACTIVE' : 'QUIET'}</span></div>{#if liveState.timeline.length}<div class="event-list" aria-label="Live events">{#each liveState.timeline.slice(-5).reverse() as event}<div class="event-row"><span class="event-kind info" aria-hidden="true">·</span><span><strong>{event.type === 'resync_required' ? 'Stream resync requested' : event.data.payload.type === 'service_status' ? `Collector ${event.data.payload.data.state}` : event.data.payload.type === 'presence_changed' ? `Presence: ${event.data.payload.data.to}` : event.data.payload.type === 'policy_changed' ? `Guard: ${event.data.payload.data.evaluation.reason.replaceAll('_', ' ')}` : 'Bandwidth updated'}</strong><small>{eventDetail(event)}</small></span><time>{event.type === 'event' ? new Date(event.data.occurred_at).toLocaleTimeString([], { hour: 'numeric', minute: '2-digit' }) : 'now'}</time></div>{/each}</div>{:else}<div class="event-quiet"><span>⌁</span><strong>Nothing needs attention</strong><p class="muted">Events will appear here as your home changes.</p></div>{/if}<CollectorStatus state={collectorState}/></aside>{/if}</div><nav class="mobile-nav" aria-label="Mobile navigation">{#each nav.slice(0, 2) as item}<button class:active={view === item[0]} type="button" onclick={() => { view = item[0]; moreOpen = false; }}><span aria-hidden="true">{item[2]}</span>{item[1]}</button>{/each}<button class:active={moreOpen} type="button" aria-expanded={moreOpen} aria-controls="mobile-drawer" onclick={() => moreOpen = !moreOpen}><span aria-hidden="true">⋯</span>More</button></nav>{#if moreOpen}<div class="mobile-drawer" id="mobile-drawer" aria-label="More destinations">{#each nav.slice(2) as item}<button class:active={view === item[0]} type="button" onclick={() => { view = item[0]; moreOpen = false; }}><span aria-hidden="true">{item[2]}</span>{item[1]}</button>{/each}</div>{/if}</div>
<style>
