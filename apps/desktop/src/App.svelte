...
<div class="app-shell"><header class="topbar"><div class="brand"><span class="brand-mark">✦</span><span>NeonHearth</span></div><nav aria-label="Primary">{#each nav as item}<button class:active={view === item[0]} type="button" onclick={() => { view = item[0]; moreOpen = false; }}><span class="nav-icon">{item[2]}</span><span>{item[1]}</span></button>{/each}</nav><span class="topbar-note"><span class="secure-icon">⌾</span><span>Local only</span></span></header><header class="mobile-head"><div class="brand"><span class="brand-mark">✦</span><span>NeonHearth</span></div><span class="kicker">{view.toUpperCase()}</span></header><div class="app-body"><main>{#if view === 'pulse'}<PulseView state={liveState}/>{:else if view === 'devices'}<DevicesView liveState={liveState}/>{:else if view === 'guard'}<GuardView state={liveState} client={cameraClient} />{:else if view === 'cameras'}{#if cameraClient}<CamerasView client={cameraClient} />{:else}<section class="not-ready"><p class="muted">Connecting to the local collector…</p></section>{/if}{:else if view === 'home'}<HomeView liveState={liveState}/>{:else if view === 'doctor'}{#if cameraClient}<DoctorView client={cameraClient} />{:else}<section class="not-ready"><p class="muted">Connecting to the local collector…</p></section>{/if}{:else}<section class="not-ready"><span class="not-ready-mark">⌁</span><p class="kicker">{view.toUpperCase()}</p><h1>This room is still being wired.</h1><p class="muted">This view is not available in this build yet. Your secure pairing and private network data remain untouched.</p></section>{/if}</main><aside class="events-rail"><div class="section-title"><h2>Live thread</h2><span class="live-pill"><i class="presence-dot {liveState.timeline.length ? 'online' : 'quiet'}" aria-hidden="true"></i> {liveState.timeline.length ? 'ACTIVE' : 'QUIET'}</span></div>{#if liveState.timeline.length}<div class="event-list" aria-label="Live events">{#each liveState.timeline.slice(-5).reverse() as event}<div class="event-row"><span class="event-kind info" aria-hidden="true">·</span><span><strong>{event.type === 'resync_required' ? 'Stream resync requested' : event.data.payload.type === 'service_status' ? `Collector ${event.data.payload.data.state}` : event.data.payload.type === 'presence_changed' ? `Presence: ${event.data.payload.data.to}` : event.data.payload.type === 'policy_changed' ? `Guard: ${event.data.payload.data.evaluation.reason.replaceAll('_', ' ')}` : 'Bandwidth updated'}</strong><small>{eventDetail(event)}</small></span><time>{event.type === 'event' ? new Date(event.data.occurred_at).toLocaleTimeString([], { hour: 'numeric', minute: '2-digit' }) : 'now'}</time></div>{/each}</div>{:else}<div class="event-quiet"><span>⌁</span><strong>Nothing needs attention</strong><p class="muted">Events will appear here as your home changes.</p></div>{/if}<CollectorStatus state={collectorState}/></aside></div><nav class="mobile-nav" aria-label="Mobile navigation">{#each nav.slice(0, 2) as item}<button class:active={view === item[0]} type="button" onclick={() => { view = item[0]; moreOpen = false; }}><span>{item[2]}</span>{item[1]}</button>{/each}<button class:active={moreOpen} type="button" aria-expanded={moreOpen} aria-controls="mobile-drawer" onclick={() => moreOpen = !moreOpen}><span>⋯</span>More</button></nav>{#if moreOpen}<div class="mobile-drawer" id="mobile-drawer" aria-label="More destinations">{#each nav.slice(2) as item}<button class:active={view === item[0]} type="button" onclick={() => { view = item[0]; moreOpen = false; }}><span>{item[2]}</span>{item[1]}</button>{/each}</div>{/if}</div>
<style>
  ...
  .events-rail .event-kind { display: inline-grid; place-items: center; width: 21px; height: 21px; border: 1px solid var(--line); border-radius: 6px; color: var(--ink-mute); font: 11px var(--font-mono); background: var(--surface); }
  .mobile-drawer { display: none; }
  @media (max-width: 850px) { .mobile-drawer { position: fixed; z-index: 6; right: 8px; bottom: 73px; display: grid; grid-template-columns: 1fr 1fr; gap: 5px; padding: 8px; background: var(--surface); border-radius: var(--radius-card); box-shadow: var(--shadow); }.mobile-drawer button { min-height: 44px; padding: 9px 12px; border: 0; border-radius: 10px; color: var(--ink-mute); background: var(--surface-2); font: 500 12.5px var(--font-body); text-align: left; cursor: pointer; }.mobile-drawer button.active { color: var(--ember); }.mobile-drawer button span { display: inline-block; width: 22px; color: var(--ink-mute); }.mobile-drawer button.active span { color: var(--ember); } }
</style>
