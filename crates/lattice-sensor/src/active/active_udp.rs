...
fn nbns(probe: &UdpProbe, b: &[u8]) -> Result<Vec<(String, String)>, ActiveError> {
    ...
    if questions > 1 || u16::from_be_bytes([b[6], b[7]]) != 1 { return Err(ActiveError::Correlation); }
    if questions == 1 {
        p = skip_name(b, p)?.checked_add(4).filter(|end| *end <= b.len()).ok_or(ActiveError::Network)?;
    }
    p = skip_name(b, p)?;
    if p + 10 > b.len() {
        return Err(ActiveError::Network);
    }
    if b[p..p + 4] != [0, 0x21, 0, 1] { return Err(ActiveError::Correlation); }
    let len = usize::from(u16::from_be_bytes([b[p + 8], b[p + 9]]));
    p += 10;
