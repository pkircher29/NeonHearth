...
pub(super) async fn negotiate(
    ...
    // MS-SMB2 3.2.4.2.2.2 reserves sequence number zero for this first request.
    // Correlation is to this dedicated TCP connection and its message ID.
    ...
    let message_id: [u8; 8] = request[24..32]
        .try_into()
    ...
    if header[0] != 0 || !(128..=16_384).contains(&size) {
...
fn parse(b: &[u8], message_id: [u8; 8]) -> Result<Vec<(String, String)>, ActiveError> {
    if b.len() < 128
        || b.len() > 16_384
        || &b[..4] != b"\xfeSMB"
        || b[4..6] != 64u16.to_le_bytes()
        || b[8..12] != [0; 4]
        || b[12..14] != [0; 2]
        || b[16] & 1 == 0
        || b[20..24] != [0; 4]
        || b[24..32] != message_id
        || b[64..66] != 65u16.to_le_bytes()
    {
    ...
    let dialect = match dialect {
        ...
        0x0300 => "SMB 3.0",
        0x0302 => "SMB 3.0.2",
        _ => return Err(ActiveError::Correlation),
    };
    let offset = u16::from_le_bytes([b[120], b[121]]) as usize;
    let length = u16::from_le_bytes([b[122], b[123]]) as usize;
    if length > 0 && (offset < 128 || offset + length > b.len()) {
        return Err(ActiveError::ResponseLimit);
    }
    Ok(vec![
        ("dialect".into(), dialect.into()),
        ("signing_required".into(), (b[66] & 2 != 0).to_string()),
        (
            "server_guid".into(),
            b[72..88].iter().map(|v| format!("{v:02x}")).collect(),
        ),
    ])
}
...
mod tests {
    ...
    fn verifies_smb_response_correlation_and_bounds() {
        let mut b = vec![0; 128];
        b[..4].copy_from_slice(b"\xfeSMB");
        b[4] = 64;
        b[16] = 1;
        b[64] = 65;
        b[66] = 3;
        b[68..70].copy_from_slice(&0x0302u16.to_le_bytes());
        assert!(
            parse(&b, [0; 8])
                .unwrap()
                .contains(&("signing_required".into(), "true".into()))
        );
        b[24] = 1;
        assert_eq!(parse(&b, [0; 8]), Err(ActiveError::Correlation));
        b[24] = 0;
        b[122] = 5;
        assert_eq!(parse(&b, [0; 8]), Err(ActiveError::ResponseLimit));
        for end in 0..128 {
            assert!(parse(&b[..end], [0; 8]).is_err());
        }
    }
}
