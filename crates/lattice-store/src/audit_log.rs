...
//! is read under the database write lock; UPDATE/DELETE are refused by SQL
//! triggers, so append-only holds even against raw SQL.
//!
...
impl AuditLog {
    ...
    pub async fn append(&self, entry: NewAuditEntry) -> Result<AppendedEntry, AuditLogError> {
        ...
        let result = append_locked(&mut conn, &entry, &occurred_at, &detail).await;
    ...
    pub async fn verify_chain(&self, scope: ChainScope) -> Result<ChainReport, AuditLogError> {
        let (root_id, root_hash) = self.trusted_root().await?;
        ...
        let (start_id, mut expected_prev, anchored) = match scope {
            ChainScope::All => (root_start, root_hash, true),
            ChainScope::Tail(0) => {
        ...
        loop {
            ...
            ) in &rows
            {
                ...
                } else if AuditActor::parse(actor).is_none()
                    || AuditCategory::parse(category).is_none()
                {
    ...
    /// stays enforced for every other writer.
    pub async fn retention_prune(
        ...
        let mut conn = begin_immediate(&self.pool).await?;
        let result = prune_locked(&mut conn, cutoff.as_deref(), max_rows, &pruned_at).await;
        finish(conn, result).await
    }

    async fn trusted_root(&self) -> Result<(i64, String), AuditLogError> {
...
/// Opens a write-locked transaction so concurrent appends/prunes serialize.
async fn begin_immediate(pool: &SqlitePool) -> Result<PoolConnection<Sqlite>, AuditLogError> {
    let mut conn = pool.acquire().await.map_err(storage)?;
...
/// Commits on success, rolls back on failure; the connection never returns to
/// the pool with an open transaction.
async fn finish<T>(
    mut conn: PoolConnection<Sqlite>,
    result: Result<T, AuditLogError>,
) -> Result<T, AuditLogError> {
    match result {
