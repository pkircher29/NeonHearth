...
fn advisory(id: &str, freshness: Freshness) -> NormalizedAdvisory {
    NormalizedAdvisory::new(AdvisoryInput {
        ...
        published_at: at(1),
        modified_at: if freshness == Freshness::FutureDated {
            Utc::now() + Duration::days(1)
        } else {
            at(2)
        },
        retrieved_at: if freshness == Freshness::FutureDated {
            Utc::now()
        } else {
            at(3)
        },
        cache_expires_at: if freshness == Freshness::Fresh {
            at(300)
        } else if freshness == Freshness::FutureDated {
...
async fn add_match(
    ...
    repo.record_match(
        ...
        lattice_advisory::RiskDimensions {
            ...
            exploitability: value.input().exploitability,
            exposure: value.input().exposure,
            confidence: value.input().confidence,
            remediation: value.input().remediation,
        },
    )
    .await
    .unwrap();
...
async fn invalid_queries_are_exactly_bad_request_and_unknown_canonical_is_not_found() {
    ...
    let (status, _) = response_json(
        &router,
        "/api/v1/devices/018f0000-0000-7000-8000-00000000abce/advisories",
    )
    .await;
...
async fn listing_isolated_deterministic_and_bounded() {
    ...
    for (id, modified) in [
        ...
        input.modified_at = at(modified);
        input.retrieved_at = at(modified);
        input.cache_expires_at = at(modified + 300);
        value = NormalizedAdvisory::new(input).unwrap();
        add_match(&pool, device, &value, Some(&identity)).await;
    }
    let mut other_value = advisory("CVE-OTHER", Freshness::Fresh);
    let mut input = other_value.input().clone();
    input.modified_at = at(400);
    input.retrieved_at = at(400);
    input.cache_expires_at = at(700);
    other_value = NormalizedAdvisory::new(input).unwrap();
    add_match(&pool, other, &other_value, Some(&identity)).await;
    let (status, json) = response_json(
        &router,
        &format!("/api/v1/devices/{device}/advisories?limit=2"),
    )
    .await;
    ...
    let (_, other_json) =
        response_json(&router, &format!("/api/v1/devices/{other}/advisories")).await;
...
async fn projection_has_exact_possible_labels_and_sanitizes_untrusted_values() {
    ...
    add_match(
        &pool,
        device,
        &advisory("CVE-EXACT", Freshness::Fresh),
        Some(&identity),
    )
    .await;
    let mut possible = advisory("CVE-POSSIBLE", Freshness::Fresh);
    let mut input = possible.input().clone();
    input.firmware = VersionConstraint::Any;
    input.source_url = "https://services.nvd.nist.gov/rest/json/cves/2.0?secret=URL_SECRET&ip=192.0.2.1&mac=AA:BB&serial=SERIAL_SECRET".into();
    possible = NormalizedAdvisory::new(input).unwrap();
    add_match(&pool, device, &possible, Some(&identity)).await;
    let (status, json) =
        response_json(&router, &format!("/api/v1/devices/{device}/advisories")).await;
    assert_eq!(status, StatusCode::OK);
    ...
    assert!(
        !text.contains("URL_SECRET")
            ...
            && !text.contains("AA:BB")
            && !text.contains("SERIAL_SECRET")
    );
    let labels: Vec<_> = json["matches"]
        .as_array()
        .unwrap()
        .iter()
    ...
    let exact = json["matches"]
        .as_array()
        .unwrap()
        .iter()
        .find(|m| m["source_id"] == "CVE-EXACT")
        .unwrap();
    assert_eq!(
        exact["matched_fields"],
        serde_json::json!(["vendor", "model", "firmware"])
    );
...
async fn freshness_and_corruption_are_projected_as_contractual_statuses() {
    ...
    for (id, freshness, modified, retrieved, expires) in [
        ...
        let mut input = value.input().clone();
        input.modified_at = modified;
        input.published_at = modified;
        input.retrieved_at = retrieved;
        input.cache_expires_at = expires;
        value = NormalizedAdvisory::new(input).unwrap();
        add_match(
            &pool,
            device,
    ...
    for (id, expected) in [
        ("CVE-FRESH", "fresh"),
        ("CVE-STALE", "stale"),
        ("CVE-FUTURE", "future_dated"),
    ] {
        let row = json["matches"]
            .as_array()
            .unwrap()
            .iter()
            .find(|m| m["source_id"] == id)
            .unwrap();
