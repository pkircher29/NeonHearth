...
use crate::discovery::{neighbor_discovery_sources, NeighborCoordinator, NeighborCoordinatorConfig,
    NeighborInterfaceBinding, PersistentDiscoveryPipeline};
use crate::{AppState, ServiceRuntimeStatus};
use chrono::Utc;
use lattice_domain::DeviceId;
use lattice_sensor::{InterfaceId, InterfaceOverride, SystemInterfaceManager};
...
pub async fn build(state: AppState, repository: M2StateRepository) -> StartupResult {
    ...
    let mut ids: Vec<InterfaceId> = inventory
        ...
        .collect();
    ids.sort();
    if ids.is_empty() { return degraded(state).await; }
    let bindings: Vec<_> = ids.iter().filter_map(|id| NeighborInterfaceBinding::for_interface(*id).ok()).collect();
    let sources = match neighbor_discovery_sources(&bindings) { Ok(s) => s, Err(_) => return degraded(state).await };
    let config = match NeighborSnapshotConfig::new(MAX_NEIGHBOR_ROWS, ids.iter().copied()) {
        Ok(c) => c, Err(_) => return degraded(state).await,
    };
    let pipeline = match PersistentDiscoveryPipeline::open(
        repository, sources, std::iter::repeat_with(DeviceId::new), Default::default(),
        FLOW_BATCH_LIMIT, LIVE_ROW_LIMIT,
    ).await { Ok(p) => p, Err(_) => return degraded(state).await };
    let source = SystemNeighborSnapshotSource::new(config);
    match NeighborCoordinator::new(source, pipeline, state.clone(), bindings, NeighborCoordinatorConfig::default()) {
...
async fn degraded(state: AppState) -> StartupResult {
    state.transition_service_status(ServiceRuntimeStatus::Degraded, Utc::now()).await;
...
pub async fn run_loop<S: lattice_sensor::neighbor::NeighborSnapshotSource>(
    coordinator: &mut NeighborCoordinator<S>, shutdown: &mut watch::Receiver<bool>,
) {
...
fn error_category(error: &crate::discovery::NeighborCoordinatorError) -> &'static str {
    match error { crate::discovery::NeighborCoordinatorError::Snapshot(_) => "snapshot", crate::discovery::NeighborCoordinatorError::Discovery(_) => "discovery" }
}
