...
| Platform paths | `cargo test -p lattice-service --test platform_contract --locked` | PASS on Linux host: 3 passed, 0 failed; Windows path contract simulated, not a Windows runtime observation |
| Windows native runtime | `cargo +stable-x86_64-pc-windows-msvc build -p lattice-service --locked` followed by [Windows PowerShell procedure](#windows-powershell-live-smoke) | PASS observed 2026-08-23: native MSVC build; temp-base `NeonHearth\\lattice.db` created; health 200, no-token state 401, authorized state 200; exactly one loopback listener during run and zero after cleanup |
...
try {
  ...
  $listenerRemained = ($remaining.Count -ne 0)
